...
def fc(x, scope, nh, *, init_scale=1.0, init_bias=0.0):
    with tf.compat.v1.variable_scope(scope):
        nin = x.get_shape()[-1] # 181
        w = tf.compat.v1.get_variable("w", [nin, nh], initializer=ortho_init(init_scale)) # 181, 64
        b = tf.compat.v1.get_variable("b", [nh], initializer=tf.constant_initializer(init_bias)) # 64
