...
def train_PPO_intrinsic(env_train, model_name, timesteps, v_mix_coef, r_in_coef, v_ex_coef, ent_coef, lr_alpha, lr_beta, rms_prop_eps, verbose, seed,preproc, n_steps,nminibatches, noptepochs,cliprange, start_intrinsic_update):
    """PPO+Intrinsic model""" 
    start = time.time()
    
    if preproc:
        
            preproc_ = PPO('MlpPolicyIntrinsicInnovationReward_PPO', env_train, v_mix_coef=v_mix_coef, v_ex_coef=v_ex_coef, r_in_coef=r_in_coef, ent_coef=ent_coef,
                        lr_alpha=lr_alpha, lr_beta=lr_beta, epsilon = rms_prop_eps, verbose=verbose, seed=seed, n_steps=1600, nminibatches = 1, noptepochs=1,cliprange=cliprange,
    ...
    model = PPO('MlpPolicyIntrinsicInnovationReward_PPO', env_train, v_mix_coef=v_mix_coef, v_ex_coef=v_ex_coef, r_in_coef=r_in_coef, ent_coef=ent_coef,
                lr_alpha=lr_alpha, lr_beta=lr_beta, epsilon = rms_prop_eps, verbose=verbose, seed=seed, n_steps=n_steps, nminibatches = nminibatches, noptepochs=noptepochs,cliprange=cliprange,
    ...
    print('Training time (PPO intrinsic): ', (end - start) / 60, ' minutes')
    return model
...
def train_PPO_no_intrinsic(env_train, model_name, timesteps, v_mix_coef, v_ex_coef, ent_coef, lr_alpha, lr_beta, rms_prop_eps, verbose, seed,preproc, n_steps,nminibatches, noptepochs,cliprange):
    """PPO Original model""" 
    start = time.time()

    if preproc:
            preproc_ = PPO('MlpPolicyIntrinsicInnovationReward_PPO', env_train, v_mix_coef=v_mix_coef, v_ex_coef=v_ex_coef, r_in_coef=0, ent_coef=ent_coef,
                        lr_alpha=lr_alpha, lr_beta=lr_beta, epsilon = rms_prop_eps, verbose=verbose, seed=seed, n_steps=1600, nminibatches = 1, noptepochs=1,cliprange=cliprange,
    ...
    model = PPO('MlpPolicyIntrinsicInnovationReward_PPO', env_train, v_mix_coef=v_mix_coef, v_ex_coef=v_ex_coef,r_in_coef=0, ent_coef=ent_coef,
                lr_alpha=lr_alpha, lr_beta=lr_beta, epsilon = rms_prop_eps, verbose=verbose, seed=seed, n_steps=n_steps, nminibatches = nminibatches, noptepochs=noptepochs,cliprange=cliprange,
...
def run_ppo_ensemble_strategy(df, unique_trade_date, rebalance_window, validation_window, timesteps,v_mix_coef, v_ex_coef, r_in_coef, ent_coef, lr_alpha, lr_beta, 
                              rms_prop_eps, verbose, seed, turb_var, preproc, n_steps, nminibatches, noptepochs,cliprange, start_intrinsic_update):
    print("============Start PPO Strategy============")
    unique_train_date = df[(df.datadate > 20081231)&(df.datadate <= 20200707)].datadate.unique()

    last_state_ensemble, last_state_ppo, last_state_org =[], [] , [] 
    model_use=[]    
    ...
    for i in range(rebalance_window + validation_window, len(unique_trade_date), rebalance_window):
        ...
        print("======PPO Intrinsic Training========")     
        
        
        model_ppo = train_PPO_intrinsic(env_train, model_name="PPOIntrinsic_{}".format(i), timesteps=timesteps, v_mix_coef=v_mix_coef, r_in_coef=r_in_coef, v_ex_coef=v_ex_coef,
                                                ent_coef=ent_coef, lr_alpha = lr_alpha, lr_beta = lr_beta, rms_prop_eps=rms_prop_eps, verbose=verbose, seed=seed, preproc=preproc, n_steps=n_steps,nminibatches=nminibatches, noptepochs=noptepochs,cliprange=cliprange, start_intrinsic_update=start_intrinsic_update) 
        
        DRL_intrinsic_validation(model=model_ppo, test_data=validation, test_env=env_val, test_obs=obs_val)
        sharpe_ppo = get_validation_sharpe(i)

                                    
        print("======PPO Original Training========")
        
        env_train = DummyVecEnv([lambda: StockEnvTrain(train,preproc=True)])
        model_ppo_org = train_PPO_no_intrinsic(env_train, model_name="PPOOriginal_{}".format(i), timesteps=timesteps, v_mix_coef=v_mix_coef, v_ex_coef=v_ex_coef, 
                                                ent_coef=ent_coef, lr_alpha = lr_alpha,lr_beta=lr_beta, rms_prop_eps=rms_prop_eps, verbose=verbose, seed=seed, preproc=preproc, n_steps=n_steps,nminibatches=nminibatches, noptepochs=noptepochs,cliprange=cliprange)
        ...
        DRL_intrinsic_validation(model=model_ppo_org, test_data=validation, test_env=env_val, test_obs=obs_val) # NONE 
        sharpe_ppo_org = get_validation_sharpe(i) 
        ...
        if sharpe_ppo >= sharpe_ppo_org:
            model_ensemble = model_ppo 
            model_use.append('PPO') 
            print("MODEL USED: PPO INTRINSIC ")

        else:
            model_ensemble = model_ppo_org
            model_use.append('PPO_ORIGINAL')
            print("MODEL USED: PPO ORIGINAL ")
        ...
        last_state_ppo  = DRL_intrinsic_prediction(df=df, model= model_ppo, name=f'IntrinsicT{timesteps//1000}S{seed}',
        ...
        last_state_org  = DRL_intrinsic_prediction(df=df, model= model_ppo_org, name=f'OriginalT{timesteps//1000}S{seed}',
...
def run_once(path, preproc=True, timesteps=30000, v_mix_coef=0.1, v_ex_coef=1, r_in_coef=0.001, ent_coef=0.01, lr_alpha=7e-4, lr_beta=7e-4, 
             rms_prop_eps=1e-5, verbose=0, seed=51104, turb_var=0.9 , n_steps = 30, start_intrinsic_update=10000,nminibatches=6, noptepochs=1,cliprange=0.2):
        ...
        #timesteps = 3000 #2000 #5000 #1000, self.terminal of envTrain hit when self.day >= 1761
        
        turbID = int(100*turb_var)
        infos , model_use = run_ppo_ensemble_strategy(data, unique_trade_date, rebalance_window, validation_window,
